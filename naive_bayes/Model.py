...
class XGen:
    ...
    # These methods let you persist col_params
    def load(self, param_filename):
        with open(param_filename, "rb") as f:
...
# class KftModel(KftModelPredictor):
#     def __init__(self, k_best_features:int, C:float, model_params:dict=None):
#         super().__init__(model_params)
#         self.k_best_features = k_best_features
#         self.C = C
#         self.X = None
#         self.selector = SelectKBest(score_func=chi2, k=self.k_best_features)
#         self.predictor = LogisticRegression(max_iter=10000, C=self.C)
#
#     @ staticmethod
#     def convert_target(t):
#         mapping = {}  # Maps original label -> numeric index
#         inverse_mapping = {}  # Maps numeric index -> original label
#         numeric_targets = []
#         current_index = 0
#
#         for label in t:
#             if label not in mapping:
#                 mapping[label] = current_index
#                 inverse_mapping[current_index] = label
#                 current_index += 1
#             numeric_targets.append(mapping[label])
#
#         return numeric_targets, inverse_mapping
#
#     def train(self, X, t):
#         t,  inverse_mapping = self.convert_target(t)
#         self.model_params["num_label_mapping"] = inverse_mapping
#         X_selected = self.selector.fit_transform(X, t)
#         selected_indices = self.selector.get_support(indices=True)
#         self.model_params["selected_feature_indices"] = selected_indices
#         self.predictor.fit(X_selected, t)
#         self.model_params["W"] = self.predictor.coef_
#         self.model_params["b"] = self.predictor.intercept_
#
#     def score(self, X, t):
#         y = self.predict(X)
#         return np.mean(y == t)
#
#     def get_params(self):
#         return {'k_best_features': self.k_best_features, 'C': self.C}
#
#     def set_params(self, **params):
#
#         for key, value in params.items():
#             if key in ['k_best_features', 'C']:
#                 setattr(self, key, value)
#             else:
#                 raise ValueError(f"Invalid parameter: {key}")
#         # Reinitialize the selector and predictor with the updated parameters.
#         self.selector = SelectKBest(score_func=chi2, k=self.k_best_features)
#         self.predictor = LogisticRegression(max_iter=10000, C=self.C)
#         return self
